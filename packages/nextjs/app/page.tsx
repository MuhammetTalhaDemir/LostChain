...
import { ArchiveBoxIcon, CheckIcon, PlusIcon, LockClosedIcon, LockOpenIcon } from "@heroicons/react/24/outline";
...
const Home: NextPage = () => {
  ...
  const handleEkle = async () => {
    try {
      ...
      setEsyaIsmi(""); setAciklama(""); setDogrulamaSorusu("");
  ...
  return (
    <div className="flex items-center flex-col grow pt-10 px-5 bg-base-200 min-h-screen">
      <div className="text-center mb-10">
        ...
        <p className="text-xl italic text-secondary font-bold underline decoration-primary">Erciyes Universitesi Seffaf Portal</p>
      </div>

      <div className="card w-full max-w-2xl bg-base-100 shadow-2xl border-2 border-primary mb-12">
        <div className="card-body">
          <h2 className="card-title text-2xl"><PlusIcon className="h-6 w-6" /> Yeni Ilan Olustur</h2>
          <div className="form-control gap-4 mt-4">
            <input type="text" placeholder="Esya Nedir?" className="input input-bordered border-primary" value={esyaIsmi} onChange={e => setEsyaIsmi(e.target.value)} />
            <textarea placeholder="Genel Aciklama" className="textarea textarea-bordered border-primary" value={aciklama} onChange={e => setAciklama(e.target.value)} />
            <input type="text" placeholder="Dogrulama Sorusu" className="input input-bordered input-secondary" value={dogrulamaSorusu} onChange={e => setDogrulamaSorusu(e.target.value)} />
            <select className="select select-bordered border-primary" onChange={e => setBulunduMu(e.target.value === "bulundu")}>
              <option value="kayip">Esyami Kaybettim</option>
              <option value="bulundu">Esya Buldum</option>
            </select>
            <button className="btn btn-primary shadow-lg" onClick={handleEkle}>Sisteme Kaydet</button>
          </div>
        </div>
      </div>

      <div className="w-full max-w-7xl">
        <h2 className="text-3xl font-bold mb-8 text-center flex justify-center gap-3"><ArchiveBoxIcon className="h-9 w-9 text-primary" /> Aktif Ilanlar</h2>
...
const EsyaKarti = ({ id, connectedAddress }: { id: number; connectedAddress?: string }) => {
  ...
  return (
    <div className={`card bg-base-100 shadow-xl border-t-8 ${durum === 2 ? "border-gray-400 opacity-60" : "border-primary"} transition-all hover:shadow-2xl`}>
      <div className="card-body">
        <div className="flex justify-between items-start">
          <div className={`badge font-bold ${durum === 0 ? "badge-error" : durum === 1 ? "badge-warning" : "badge-success"}`}>
        ...
        {durum === 0 && !isSahibi && (
          <div className="mt-4 p-4 bg-base-200 rounded-2xl border border-dashed border-primary">
            ...
            <input type="text" placeholder="Iletisim (Tel/TG)" className="input input-xs input-bordered w-full mb-2" onChange={e => setIletisim(e.target.value)} />
            <input type="text" placeholder="Gizli Konum Bilgisi" className="input input-xs input-bordered w-full mb-2" onChange={e => setKonum(e.target.value)} />
            <button className="btn btn-warning btn-xs w-full" onClick={() => writeYourContractAsync({ functionName: "buldugunuBildir", args: [BigInt(id), iletisim, konum] })}>Buldum!</button>
          </div>
        )}
        ...
        {durum === 1 && isSahibi && (
          <div className="mt-4 p-4 bg-primary/10 rounded-2xl border border-primary text-center">
            ...
            <button className="btn btn-primary btn-xs w-full" onClick={() => writeYourContractAsync({ functionName: "kisiOnayla", args: [BigInt(id), bulanKisi] })}>BU KISIYI ONAYLA</button>
          </div>
        )}

        <div className="mt-6 p-4 rounded-2xl bg-base-300 border-2 border-base-content/10">
          <div className="flex items-center gap-2 mb-2">
            {isOnayli || isSahibi ? <LockOpenIcon className="h-5 w-5 text-success" /> : <LockClosedIcon className="h-5 w-5 text-error" />}
          ...
          {isOnayli || isSahibi ? (
            ...
            <p className="text-[10px] italic opacity-50 text-center">Bu bilgiye sadece ilani veren kisinin onayladigi cuzdan erisebilir.</p>
          )}
        </div>
        ...
        <div className="card-actions justify-end mt-6 border-t pt-4">
          ...
          {isSahibi && durum !== 2 && (
            <button className="btn btn-success btn-sm w-full mt-4 gap-2" onClick={() => writeYourContractAsync({ functionName: "teslimEdildiIsaretle", args: [BigInt(id)] })}>
